...
class Manejador:
    ...
    def modificarBasico(self, dni, nuevoBasico):
        actual = self.__comienzo
        band = True
        while actual is not None and band:
            agente = actual.getDato()
        ...
        while actual is not None and band:
            ...
            if agente.getCuil() == dni:
                if isinstance(agente, Docente):
                    agente.setPorcentaje(nuevoPorcentaje)
                    band = False
            actual = actual.getSiguiente()
        if band:
    ...
    def gastosSueldoPorEmpleado(self, dni):
        ...
        while actual is not None and band:
            ...
            if agente.getCuil() == dni:
                band = False
                print(f'El sueldo del empleado de dni {dni} es ${agente.calcularSueldo()}')
            actual = actual.getSiguiente()
        if band:
            print('No se encontró el agente')
        ...
        while actual is not None and band:
            ...
            if agente.getCuil() == dni:
                ...
                if isinstance(agente, DocInvestigador):
                    agente.setImporte(nuevoImporte)
                    print('Carga exitosa')
                else:
